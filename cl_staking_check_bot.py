...
async def capture_screenshot(url, output_file):
    ...
    print(f"Opening {url}...")
    try:
        # Add extra launch args to help avoid black screenshots in some CI environments
        ...
        if os.path.exists(output_file):
            ...
            print(f"Screenshot file size: {size_bytes} bytes")
            if size_bytes == 0:
                print("Warning: Screenshot file is zero bytes!")
        else:
            print("Warning: Screenshot file not found after capture.")
...
def crop_screenshot(input_file, output_file, crop_box):
    ...
    try:
        ...
        if os.path.exists(output_file):
            size_bytes = os.path.getsize(output_file)
            print(f"Cropped file size: {size_bytes} bytes")
            if size_bytes == 0:
                print("Warning: Cropped screenshot file is zero bytes!")
        else:
            print("Warning: Cropped screenshot file not found.")
...
def post_to_twitter(message, media_file, auth):
    ...
    if media_file and os.path.exists(media_file):
        ...
        print(f"Attempting to upload media: {media_file} ({media_size} bytes)")
        if media_size > 0:
            try:
            ...
            print("Media file is empty. Skipping upload.")
    else:
        print(f"Media file does not exist or was not provided: {media_file}")
...
def fetch_staking_data():
    url = (
        "https://staking.chain.link/api/query?"
        "query=STAKING_POOL_TOTAL_PRINCIPAL_QUERY&"
        "variables=%7B%22schemaName%22%3A%22ethereum-mainnet%22%2C"
        "%22contractAddress%22%3A%220xBc10f2E862ED4502144c7d632a3459F49DFCDB5e%22%7D"
    )
    try:
...
def main():
    ...
    screenshot_success = asyncio.get_event_loop().run_until_complete(
    ...
    message = (
        f"⚠️ LINK Staking pool alert: {available_for_staking:,} LINK available for staking.\n"
